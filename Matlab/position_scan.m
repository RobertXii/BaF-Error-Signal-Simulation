...
W_matrix  = zeros(nnr, nL2);
a0_matrix = zeros(nnr, nL2);
a1_matrix = zeros(nnr, nL2);
...
for i = 1:nnr
    for j = 1:nL2
        ...
        fitResults = sim.fitAsymmetry(false);
...
rowNames = compose('%d', nr_pos);
varNames = compose('%d', L2_pos);
...
% Plot the population evolution and asymmetry curves for visualization.
%
% The following section plots the population evolution for the first
% simulation run. Change the method or index as needed. Yes/No implementation
% of 2 sections TDPT fit.
%
