...
% With 2,2 unitary transformation as exp(i*omega_stark*t)
% H_prime = @(t) [
%     0, (1i * W + d12 * E_t(t)) * exp(-1i * omega_stark * t), d13 * E_t(t) * exp(-1i * omega_L2 * t);
%     (-1i * W + d12 * E_t(t)) * exp(1i * omega_stark * t), Delta - omega_stark, d23 * E_t(t) * exp(-1i * (omega_L2 - omega_stark) * t);
%     d13 * E_t(t) * exp(1i * omega_L2 * t), d23 * E_t(t) * exp(1i * (omega_L2 - omega_stark) * t), Delta13 - omega_L2
% ];

% With 2,2 unitary transformation as exp(i * Delta * t)
H_prime = @(t) [
    0, (1i * W + d12 * E_t(t)) * exp(-1i * Delta * t), d13 * E_t(t) * exp(-1i * omega_L2 * t);
    (-1i * W + d12 * E_t(t)) * exp(1i * Delta * t), Delta - Delta, d23 * E_t(t) * exp(-1i * (omega_L2 - Delta) * t);
    d13 * E_t(t) * exp(1i * omega_L2 * t), d23 * E_t(t) * exp(1i * (omega_L2 - Delta) * t), Delta13 - omega_L2
];
