%% Parameters and Initialization
% Constants
W = 0;                              % Weak interaction matrix element [rad/s]
Delta = 2 * pi * 2e3;               % Energy gap for |b> [rad/s]
Delta13 = 2 * pi * 348.69e12;       % Energy gap for |c> [rad/s]
d12 = -33.6 * 2 * pi;               % Dipole moment between ab  [rad/(s·V/m)]
d13 = 33.6 * 2 * pi;                % Dipole moment between ac  [rad/(s·V/m)]
d23 = 0;                            % Dipole moment between bc  [rad/(s·V/m)]
gamma_c = 2 * pi * 2.7e7;           % Decay rate for |c| in [rad/s]

%Field parameters
E0_stark = 10;                      % Stark field amplitude [V/m]
E0_nr = 42;                         % Non-reversing field amplitude [V/m]
E0_L2 = 8.514e5;                    % Depletion laser L2 amplitude [V/m]
omega_stark = 2 * pi * 11.4e3;      % Stark field frequency [rad/s]
detuning_L2 = 2 * pi * 1e6 * -3;     % L2 laser detuning [rad/s]
v = 616;                            % Molecular beam velocity [m/s]
sigma_u = 0.76e-2;                  % Width constant of the non-reversing field [m]
t0 = 70.1e-6;                       % Center of the non-reversing field [s]
tspan = linspace(-51.1e-6, 100e-6, 100000); % Time span for the simulation [s]

% Define variables
% detuning_range = linspace(-4000, 4000, 200) * 2 * pi; % Detuning values in rad/s
% detuning_L2_range = linspace(-4e6, 4e6, 16) * 2 * pi; % Detuning values in rad/s
% omega_L2 = Delta13 + detuning_L2_range; 
E0_nr_range = linspace(-12, 12, 15);

% Initialize asymmetry array
asymmetry = zeros(size(E0_nr_range));
W_val = zeros(size(E0_nr_range));

%% Initial Condition
% Initial state
rho0 = zeros(3);
rho0(1,1) = 1; % Start in state |a>
rho0(2,2) = 0; % Start in state |a>

%% Start the parallel computing
% Enable parallel pool
if isempty(gcp('nocreate'))
    parpool; % Start a parallel pool if one is not running
end

% Parallel loop for detuning range
parfor i = 1:length(E0_nr_range)
    % detuning_L2 = detuning_L2_range(i);  % Current detuning
    E0_nr = E0_nr_range(i); 
    omega_L2 = Delta13 + detuning_L2;      % Laser L2 frequency
    diff = omega_L2 - omega_stark; % energy difference between 2, 3

    % Define electric fields as functions of time
    E_stark = @(t) (t > -43.7e-6 & t < 43.7e-6) .* (E0_stark * sin(omega_stark * (t + 43.7e-6)));
    E_nr = @(t) E0_nr * sech(v * (t - t0) / sigma_u);
    E_L2 = @(t) (t > 52.35e-6 & t < 52.85e-6) .* (E0_L2 * sin(omega_L2 * (t - 52.35e-6)));
    
    E_t = @(t) E_stark(t) + E_nr(t) + E_L2(t); % Total field
    E_t_reversed = @(t) - E_stark(t) + E_nr(t) + E_L2(t); % Total reversed field

    % Define Hamiltonians for forward and reversed fields
    H_prime = @(t) [
        0, (1i * W + d12 * E_t(t)) * exp(-1i * omega_stark * t), d13 * E_t(t) * exp(-1i * omega_L2 * t);
        (-1i * W + d12 * E_t(t)) * exp(1i * omega_stark * t), Delta - omega_stark, d23 * E_t(t) * exp(-1i * diff * t);
        d13 * E_t(t) * exp(1i * omega_L2 * t), d23 * E_t(t) * exp(1i * diff * t), -detuning_L2
    ];

    H_prime_reversed = @(t) [
        0, (1i * W + d12 * E_t_reversed(t)) * exp(-1i * omega_stark * t), d13 * E_t_reversed(t) * exp(-1i * omega_L2 * t);
        (-1i * W + d12 * E_t_reversed(t)) * exp(1i * omega_stark * t), Delta - omega_stark, d23 * E_t_reversed(t) * exp(-1i * diff * t);
        d13 * E_t_reversed(t) * exp(1i * omega_L2 * t), d23 * E_t_reversed(t) * exp(1i * diff * t), -detuning_L2
    ];

    % Solve the ODE for forward and reversed fields
    options = odeset('RelTol', 1e-18, 'AbsTol', 1e-20); % Tighten tolerances
    [~, rho_sol] = ode45(@(t, rho_vec) master_equation(t, rho_vec, H_prime, gamma_c), tspan, rho0(:));
    [~, rho_sol_reversed] = ode45(@(t, rho_vec) master_equation(t, rho_vec, H_prime_reversed, gamma_c), tspan, rho0(:));
    
    % Extract the final density matrices
    rho_final = reshape(rho_sol(end, :), 3, 3); % Final density matrix for forward field
    rho_final_reversed = reshape(rho_sol_reversed(end, :), 3, 3); % Final density matrix for reversed field
    
    % Calculate populations
    S_plus = real(rho_final(2,2)); % Population of |b> for forward field
    S_minus = real(rho_final_reversed(2,2)); % Population of |b> for reversed field
    
    % Calculate the asymmetry
    asymmetry(i) = (S_plus - S_minus) / (S_plus + S_minus);
    W_val(i) = d12 * Delta * E0_stark * asymmetry(i) / (2 * omega_stark * 2 * pi);
    % format long;
    % disp(omega_L2);

    rho_solutions{i} = rho_sol; % Store the time evolution for each detuning
end

%% Visualization the Result

% Convert detuning to Hz for plotting
% detuning_Hz = detuning_L2_range / (2 * pi);

% Plot the asymmetry vs detuning
figure;
scatter(E0_nr_range * 10, W_val, 50, 'k', 'filled'); % Example size and style
% xlabel('\Delta / 2\pi [kHz]'  );
% xlabel('\delta_{L2} / 2\pi [MHz]');
xlabel('E_{nr0} [mV/cm]');
ylabel('W/2\pi (Hz)');
title('W vs. E_{nr0}');
set(gca, 'FontName', 'Times New Roman', 'FontSize', 15);
grid on;

rho_sol_mid = rho_solutions{8};
time = tspan;
pop_a = real(rho_sol_mid(:, 1)); % Population of |a> (rho_aa)
pop_b = real(rho_sol_mid(:, 5)); % Population of |b> (rho_bb)
pop_c = real(rho_sol_mid(:, 9)); % Population of |c> (rho_cc)
figure;
plot(time/1e6, pop_a, 'g', 'LineWidth', 1.5); % Red for |a>
hold on;
plot(time/1e6, pop_b, 'r', 'LineWidth', 1.5); % Green for |b>
plot(time/1e6, pop_c, 'b', 'LineWidth', 1.5); % Blue for |c>

% Add labels and legend
xlabel('Time [\mus]');
ylabel('Population');
title('Time Evolution of Energy State Populations');
legend('|a>', '|b>', '|c>', 'Location', 'Best');
grid on;
set(gca, 'FontName', 'Times New Roman', 'FontSize', 15);

%% Define the Master equation
% Master equation function
function drho_dt = master_equation(t, rho_vec, H_prime, gamma_c)
    rho = reshape(rho_vec, 3, 3); % Reshape vector to 3x3 matrix
    % Evaluate the time-dependent Hamiltonian
    H = H_prime(t);
    % Commutator term
    commutator = -1i * (H * rho - rho * H);
    % Dissipator term
    dissipator =  - gamma_c / 2  * [0, 0, rho(1,3); 0, 0, rho(2,3); rho(3,1), rho(3,2), 2 * rho(3,3)];  
    % Total derivative
    drho_dt = commutator + dissipator; % No dissipator since gamma_c = 0
    drho_dt = drho_dt(:); % Flatten to a vector for the solver
end


